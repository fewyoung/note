...
def login():			
	...
	if modify_form.modifysubmit.data:
		if (modify_form.oldpassword.data == ''
			or modify_form.newpassword.data == ''
			or modify_form.confirmpassword.data == ''):
			flash('错误：旧密码、新密码和确认新密码不能为空')
	if modify_form.modifysubmit.data and modify_form.validate_on_submit():	
		user = 	User.query.filter_by(id=int(get_id()))
		print(user)
